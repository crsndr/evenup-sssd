...
describe 'sssd', :type => :class do
  ...
  context 'ldap' do
    ...
    context 'setting ldap_group_search_base' do
      ...
      it { should contain_file('/etc/sssd/sssd.conf').with_content(/ldap_group_search_base = dc=Group,dc=org/)}
    end

   context 'setting ldap_user_search_base' do
      let(:params) { { :ldap_user_search_base => 'dc=User,dc=org' } }
      it { should contain_file('/etc/sssd/sssd.conf').with_content(/ldap_user_search_base = dc=User,dc=org/)}
    end
  end
